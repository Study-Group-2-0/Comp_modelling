...
```{r}
#loading packages
pacman::p_load(tidyverse, brms, metafor, here, patchwork, wesanderson, RColorBrewer)
```
...
ggplot(meta_no_na, 
       aes(
         ...
         y = ID)) + 
  geom_pointrange(
    size = .2, aes(color = meta_no_na$Language, stroke = 1.25)) +
  scale_color_brewer(palette = 'Paired') +
  geom_vline(xintercept = 0, size = .25, lty = 2) +
  geom_text(
    data = mutate_if(meta_no_na, is.numeric, round, 2), size = 2.5, 
...
m0_meta_prior <- brm(
  ...
  chains = 2,
  cores = 2,
  file = here("Assignment_3", "models_final", "m0_meta_prior")
...
m0_meta <- brm(
  ...
  chains = 2,
  cores = 2,
  file = here("Assignment_3", "models_final", "m0_meta_full")
...
# Finding priors for the language and id
# Language
sum_1 <- df %>% group_by(Language) %>% summarise(mean = mean(Pitch_IQR),
                                        sd = sd(Pitch_IQR))
sd(sum_1$mean)
# the sd for language = 0.70

# Id
sum_2 <- df %>% group_by(ID) %>% summarise(mean = mean(Pitch_IQR),
                                        sd = sd(Pitch_IQR))
sd(sum_2$mean) 
# The sd for id = 0.83
...
m2_no_meta_priors <- c(
  ...
  prior(normal(0, 0.83), class = sd, group = Language:ID),
  prior(normal(0, 0.70), class = sd, group = Language)
)
...
m2_no_meta_prior <- brm(
  ...
  cores = 1,
  backend = "cmdstanr",
  file = here("Assignment_3", "models_final", "m2_no_meta_prior")
  )
...
m2_no_meta <- brm(
  ...
  backend = "cmdstanr",
  file = here("Assignment_3", "models_final", "m2_no_meta_full")
)
...
#doing posterior-predictive checks
pp1 <- pp_check(m2_no_meta, nsamples = 100) + labs(title = "Without meta priors")
pp1
#Gaussian is maybe not best fit, but works quite fine anyways

#maybe use these???
plot(conditional_effects(m2_no_meta, spaghetti=F, 
...
m2_no_meta_samples %>% 
  ...
  labs(title = "Prior-posterior updates for model without meta-analytic priors")+
  theme_bw()+
...
D3 <- m2_no_meta_samples %>% 
  ...
  labs(title = "Prior-posterior updates for Diagnosis model without meta-analytic priors")+
  theme_bw()+
  scale_fill_manual(values = wes_palette(name = "Cavalcanti1")) + 
  theme(strip.text = element_text(size=15),
        axis.title=element_text(size=15),
...
hypothesis(m2_no_meta, "Languagedk < Languageus")
plot(hypothesis(m2_no_meta, "Languagedk < Languageus"))

# Significant difference between the two effects? 
hypothesis(m2_no_meta, "Languageus:DiagnosisTD < Languagedk:DiagnosisTD ")
plot(hypothesis(m2_no_meta, "Languageus:DiagnosisTD < Languagedk:DiagnosisTD"))

# Significant effect of Diagnosis? 
hypothesis(m2_no_meta, "(Languageus:DiagnosisTD + Languagedk:DiagnosisTD)/2 < 0 ")
plot(hypothesis(m2_no_meta, "(Languageus:DiagnosisTD + Languagedk:DiagnosisTD)/2 < 0 "))
```
...
```{r}
########################## MODEL WITH META-ANALYSIS #################################
##################### DEFINING MODEL AND MAKING PRIOR-PREDICTIVE CHECKS ####################
# SIGRID

#defining priors
m2_meta_priors <- c(
  prior(normal(-0.46,0.11), class = b, coef = Languagedk:DiagnosisTD), # meta priors
  prior(normal(-0.46,0.11), class = b, coef = Languageus:DiagnosisTD), # meta priors
  prior(normal(0.25,1), class = b, coef = Languagedk),  # meta priors
  prior(normal(0.25,1), class = b, coef = Languageus),  # meta priors
  prior(exponential(1), class = sigma),
  
  prior(normal(0, 0.83), class = sd, group = Language:ID),
  prior(normal(0.35, 0.1), class = sd, group = Language) # meta priors
)
...
m2_meta_prior <- brm(
  ...
  cores = 4,
  backend = "cmdstanr",
  file = here("Assignment_3", "models_final", "m2_meta_prior")
)
...
m2_meta <- brm(
  ...
  cores = 4, #changer to 1 if it is the first round
  backend = "cmdstanr",
  file = here("Assignment_3", "models_final", "m2_meta_full")
)
```
...
#Diagnostics
print(m2_meta)
#There is a good amount of EES and RHAT good
# No warnings
...
#doing posterior-predictive checks
pp2 <- pp_check(m2_meta, nsamples = 100) + labs(title = "With meta priors")
pp2
#Gaussian is maybe not best fit, but works quite fine anyways
...
m2_meta_samples %>% 
  ...
  geom_density(alpha = 0.8) +
  facet_wrap(~Parameter, nrow = 4) +
  labs(title = "Prior-posterior updates in model with meta-analytic priors")+
  theme_bw()+
  scale_fill_manual(values = wes_palette(name = "Cavalcanti1")) + 
  theme(strip.text = element_text(size=15),
        axis.title=element_text(size=15),
...
D4 <- m2_meta_samples %>% 
  ...
  geom_density(alpha = 0.8) +
  facet_wrap(~Parameter, nrow = ) +
  labs(title = "Prior-posterior updates for Diagnosis in model with meta-analytic priors")+
  theme_bw()+
  scale_fill_manual(values = wes_palette(name = "Cavalcanti1")) + 
  theme(strip.text = element_text(size=15),
        axis.title=element_text(size=15),
...
#saveRDS(m2_meta, file = "m2_meta_full.rds")
```
...
# Significant difference between DK and US?
hypothesis(m2_meta, "Languagedk < Languageus")
plot(hypothesis(m2_meta, "Languagedk < Languageus"))

# Significant difference between the two effects? 
hypothesis(m2_meta, "Languageus:DiagnosisTD < Languagedk:DiagnosisTD ")
plot(hypothesis(m2_meta, "Languageus:DiagnosisTD < Languagedk:DiagnosisTD"))

# Significant effect of Diagnosis? 
hypothesis(m2_meta, "(Languageus:DiagnosisTD + Languagedk:DiagnosisTD)/2 < 0 ")
plot(hypothesis(m2_meta, "(Languageus:DiagnosisTD + Languagedk:DiagnosisTD)/2 < 0 "))
```
...
```{r}
# SIGRID
# Posterior predictive checks in the two models
pp1 + pp2 + plot_annotation(title = "Posterior predictions for the two models")
```
```{r, fig.height= 10, fig.width=10}
################# Plotting posteriors from the two models #############################
# adding a model variable to the two samples
m2_meta_samples <- m2_meta_samples %>% mutate(model = "With meta-priors")
m2_no_meta_samples <- m2_no_meta_samples %>%  mutate(model = "Without meta-priors")
# Combining the two samples to one df
m2_samples <- rbind(m2_meta_samples, m2_no_meta_samples)


# Plotting the posteriors from the two models
m2_samples %>% 
  select(c("b_Languagedk", "b_Languageus", "b_Languagedk:DiagnosisTD", "b_Languageus:DiagnosisTD", "sd_Language__Intercept", "sd_Language:ID__Intercept", "model")) %>% 
  rename(
    b_sdLanguage = sd_Language__Intercept,
    b_sdLanguageID = "sd_Language:ID__Intercept",
  ) %>% 
  ...
  geom_density(alpha = 0.8) +
  facet_wrap(~Parameter, nrow = 4) +
  labs(title = "Posterior distributions for models with and without meta priors")+
  theme_bw()+
  scale_fill_manual(values = wes_palette(name = "Cavalcanti1")) + 
  theme(strip.text = element_text(size=15),
        axis.title=element_text(size=15),
...
## loo comparison
loo_compare(m2_meta, m2_no_meta)
loo_model_weights(m2_meta, m2_no_meta)
```
