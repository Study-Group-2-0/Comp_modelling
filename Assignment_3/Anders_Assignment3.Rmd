...
#Rhat is 1 for all but Intercept, for which it is 1.01
...
m1_samples <- posterior_samples(m1_no_meta)
m1_samples %>% 
...
m1_samples %>% 
  ...
  geom_density(aes(b_DiagnosisTD), fill="blue", alpha=0.3)
...
#priors look the same as no_meta_model because we have only changed priors for b.
```
```{r}
######################################### MODELLING PITCH VARIABILITY FROM DIAGNOSIS #################################################
###################################################### WITH META-ANALYSIS #########################################################
...
#--------------------------------prior-posterior-update-------------------------
#Prior posterior update checks
m1_samples_meta <- posterior_samples(m1_meta)
m1_samples_meta %>% 
  select(c("b_Intercept", "b_DiagnosisTD", "sigma", "prior_Intercept", "prior_b", "prior_sigma")) %>% 
  rename(
    b_sigma = sigma,
...
#------------------------------influential data points and LOO------------------
#check for influential data-points
m1_no_meta <- add_criterion(m1_meta, "loo", reloo = T)
 #5 problematic observation(s) found.
#The model will be refit 5 times.
