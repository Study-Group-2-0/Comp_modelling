...
ggplot(meta_no_na, aes(x = EffectSize, xmin = (EffectSize + StandardError), xmax = (EffectSize - StandardError),y = ID)) + 
  geom_pointrange(size = .2, aes(color = meta_no_na$Language)) +
...
m2_no_meta_prior <- brm(
  ...
  cores = 4,
  backend = "cmdstanr",
  file = here("Assignment_3", "models_gustav", "m2_no_meta_prior")
  )
...
m2_no_meta <- brm(
  ...
  backend = "cmdstanr",
  file = here("Assignment_3", "models_gustav", "m2_no_meta_full")
)
...
```{r}
hep <- tibble(
  sdLanguage_prior_noLog = rnorm(1e4, 0, 0.70),
  sdLanguageID_prior_noLog = rnorm(1e4, 0, 0.83),
  sdLanguage_prior_log = log(sdLanguage_prior_noLog),
  sdLanguageID_prior_log = log(sdLanguageID_prior_noLog) 
) %>% 
  pivot_longer(cols = everything(),
               names_to = "Distribution") %>% 
  separate(col = Distribution, c("Parameter","Prior","Transformation"), sep = "_")

hep %>% ggplot()+
  aes(value, fill = Transformation)+
  geom_density()+
...
m2_meta_prior <- brm(
  ...
  cores = 4,
  backend = "cmdstanr",
  file = here("Assignment_3", "models_gustav", "m2_meta_prior")
)

# 
summary(m2_meta_prior)
#doing prior_predictive checks
...
m2_meta <- brm(
  ...
  cores = 4, #changer to 1 if it is the first round
  backend = "cmdstanr",
  file = here("Assignment_3", "models_gustav", "m2_meta_full")
)
...
```{r}
df %>% group_by(Language) %>% summarise(n())
...
```{r}
##
loo_compare(m2_no_meta, m2_meta)
loo_model_weights(m2_no_meta, m2_meta)
```
