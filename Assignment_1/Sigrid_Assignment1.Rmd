...
df_all_4 <- tibble(
  pgrid = seq(0, 1, length.out = dens), # making grid approximation
  #prior = dnorm(pgrid, 0.2, 0.05) # low prior
...
```{r}
sum(HPDI(df_all_2$pgrid, prob = df_all_2$Riccardo_post)) # not correct
mean(df_all_2$Kristian_post)
mean(df_all_2$Daina_post)
mean(df_all_2$Mikkel_post)
