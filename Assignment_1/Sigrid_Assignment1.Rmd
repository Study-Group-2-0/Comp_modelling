...
bayesian_ric_50 <- filter(bayesian_ric, bayesian_ric$pgrid > 0.5)
sum(bayesian_ric_50$posterior)

# det kunne jeg have sagt mig selv. 50% sandsynlighed..
...
ggplot(df_all_long, aes(pgrid, value, colour = variable)) +
 geom_line(size = 1) + theme_minimal() + geom_line(aes(pgrid, prior/dens), color = 'dark blue') + xlab('Proportion of correctly answered questions (pgrid) ') + ylab('Posterior probability') + labs(title = 'Knowledge of CogSci', subtitle = 'How many questions can you get right?') + 
...
# a nicer version
ggplot(df_all_long_2, aes(pgrid, value, colour = variable)) +
 geom_line(size = 1) + theme_minimal() + geom_line(aes(pgrid, prior/dens), color = 'dark blue') + xlab('Proportion of correctly answered questions (pgrid) ') + ylab('Posterior probability') + labs(title = 'Knowledge of CogSci', subtitle = 'How many questions can you get right?') + 
  theme(legend.position = 'bottom', legend.title = element_blank()) + 
  scale_colour_hue(labels = c('Riccardo', 'Kristian', 'Daina', 'Mikkel'))
...
```
*As the amount of data increase, the 'tops' become higher and more pointy. That is, the model is more certain that a teacher will have a given posterior probability.*
